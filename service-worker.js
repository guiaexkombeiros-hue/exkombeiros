...
self.addEventListener('fetch', function (event) {
  ...
  // Network-first para index.html — sempre busca versão mais recente quando online
  var ehIndex = url.endsWith('/') || url.endsWith('/index.html') || url.indexOf('index.html') !== -1;
  if (ehIndex) {
    event.respondWith(
      fetch(event.request).then(function(respostaRede) {
        ...
        return caches.match(event.request); // offline: usa cache
      })
    );
    return;
  }

  // Cache-first para demais arquivos (imagens, ícones, selos)
  event.respondWith(
